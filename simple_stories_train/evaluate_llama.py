...
Arguments:
    ...
    tokenizer_eos_token (str): End-of-sequence token for the tokenizer
...
from simple_stories_train.utils import load_config
from simple_stories_train.train_llama import Config
...
class HuggingFaceStoriesDataset(Dataset):
    ...
    def filter(self, predicate: callable) -> 'HuggingFaceStoriesDataset':
...
def main(model_path: str, model_config: str, tokenizer_path: str, 
         tokenizer_eos_token: str,  device: str = "cpu"):
    model = get_model(
        "simple_stories/",
    ...
    eval(evaluate_story_generation, model=model, limit=1, max_tokens=300)
